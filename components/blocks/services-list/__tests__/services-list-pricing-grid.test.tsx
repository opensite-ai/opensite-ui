...
import { ServicesListPricingGrid } from "../services-list-pricing-grid";
...
describe("ServicesListPricingGrid", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("routes truthy icons through DynamicIcon and preserves fallback precedence", () => {
    ...
    expect(rawIcon).toHaveClass("h-6", "w-6", "text-primary");
    ...
    ).toHaveClass("h-6", "w-6", "text-primary");
    expect(
    ...
    expect(rawCard?.querySelector(".rounded-lg.p-3")).toBeInTheDocument();
    expect(noIconCard?.querySelector(".rounded-lg.p-3")).toBeInTheDocument();
    expect(
      noIconCard?.querySelector('[data-testid^="mock-icon-"]'),
    ).not.toBeInTheDocument();
  });

  it("preserves duration, item, and CTA static icons", () => {
    render(
      <ServicesListPricingGrid
        services={[
          {
    ...
    expect(screen.getByTestId("mock-icon-lucide/clock")).toHaveClass(
      "h-4",
      "w-4",
    ...
    expect(screen.getByTestId("mock-icon-lucide/arrow-right")).toHaveClass(
      "ml-2",
      "h-4",
      "w-4",
    );
