...
import { ServicesListAccordion } from "../services-list-accordion";
...
describe("ServicesListAccordion", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("routes truthy icons through DynamicIcon and preserves persistent wrappers", () => {
    ...
    const rawService = container.querySelector(".raw-service") as HTMLElement;
    const rawIcon = within(
      rawService.querySelector(".icon-shell") as HTMLElement,
    ).getByTestId("mock-icon");
    expect(rawIcon).toHaveAttribute("data-name", "lucide/sparkles");
    expect(rawIcon).toHaveClass("h-5", "w-5");
    expect(
      within(rawService).queryByText("lucide/sparkles"),
    ).not.toBeInTheDocument();
    expect(
      within(
        container.querySelector(".custom-service") as HTMLElement,
  ...
  it("preserves truthy service slots and falsy slot fallback", () => {
    ...
    const { rerender } = render(
      <ServicesListAccordion services={services} servicesSlot={false} />,
    );
  ...
  it("keeps accordion copy and deliverables on their content paths", () => {
    const { container } = render(
      <ServicesListAccordion
        defaultValue="item-0"
        services={[
    ...
    expect(screen.getByText("lucide/deliverable-copy")).toBeInTheDocument();
    expect(
      container.querySelector('[data-name="lucide/item-copy"]'),
    ).not.toBeInTheDocument();
    expect(
      container.querySelector('[data-name="lucide/deliverable-copy"]'),
    ).not.toBeInTheDocument();
    expect(
      container.querySelector('[data-name="lucide/chevron-down"]'),
    ).toBeInTheDocument();
  });
});
