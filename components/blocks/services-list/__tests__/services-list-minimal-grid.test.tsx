...
describe("ServicesListMinimalGrid", () => {
  ...
  it("routes truthy icons through DynamicIcon and preserves fallback precedence", () => {
    ...
    const falsyIconCard = container.querySelector(".falsy-icon-service");
    expect(rawCard?.querySelector(".h-12.w-12")).toBeInTheDocument();
    expect(noIconCard?.querySelector(".h-12.w-12")).not.toBeInTheDocument();
    expect(falsyIconCard?.querySelector(".h-12.w-12")).not.toBeInTheDocument();
  });

  it("preserves the CTA static icon", () => {
    render(
      <ServicesListMinimalGrid
        services={[
          {
