...
import type { SectionBackground, SectionSpacing, ActionConfig, OptixFlowConfig } from "../../../src/types";
...
export function ServicesListCultureTabs({
  ...
  const renderAspects = () => {
    ...
    return (
      <Tabs defaultValue={activeDefaultValue} className={cn("space-y-8", tabsClassName)}>
              ...
              <div className="space-y-4">
                {aspect.title && (
                  typeof aspect.title === "string" ? (
                    ...
                    <div className="text-2xl font-bold tracking-tight">{aspect.title}</div>
                  )
                )}
                {aspect.description && (
                  typeof aspect.description === "string" ? (
                    ...
                    <div className="text-muted-foreground leading-relaxed">{aspect.description}</div>
                  )
                )}
              </div>
              ...
              {aspect.testimonial && (
                <Card className="border-0 bg-linear-to-br from-primary/5 to-primary/10 p-0">
                  <CardContent className="space-y-4 p-6">
                    <DynamicIcon name="lucide/quote" className="h-8 w-8 text-primary/40" />
                    {aspect.testimonial.quote && (
                      typeof aspect.testimonial.quote === "string" ? (
                        <p className="italic text-muted-foreground">&quot;{aspect.testimonial.quote}&quot;</p>
                      ) : (
                        <div className="italic text-muted-foreground">{aspect.testimonial.quote}</div>
                      )
                    )}
                      ...
                      <div>
                        {aspect.testimonial.author && (
                          typeof aspect.testimonial.author === "string" ? (
                            <h4 className="text-sm font-medium">{aspect.testimonial.author}</h4>
                          ) : (
                            <div className="text-sm font-medium">{aspect.testimonial.author}</div>
                          )
                        )}
                        {aspect.testimonial.role && (
                          typeof aspect.testimonial.role === "string" ? (
                            <p className="text-xs text-muted-foreground">{aspect.testimonial.role}</p>
                          ) : (
                            <div className="text-xs text-muted-foreground">{aspect.testimonial.role}</div>
                          )
                        )}
                      </div>
                    </div>
  ...
  const renderCta = () => {
    ...
    if (!ctaTitle && !ctaDescription && !primaryAction && !secondaryAction) return null;

    return (
      <div className={cn("relative rounded-xl border bg-background p-8 md:p-12", ctaClassName)}>
          ...
          <div>
            ...
            {ctaDescription && (
              typeof ctaDescription === "string" ? (
                ...
                <div className="mb-6 text-muted-foreground">{ctaDescription}</div>
              )
            )}
            <div className="flex gap-4">
              {primaryAction && (
                <Pressable href={primaryAction.href} onClick={primaryAction.onClick} variant="default" asButton>
                  {primaryAction.label}
                </Pressable>
              )}
              {secondaryAction && (
                <Pressable href={secondaryAction.href} onClick={secondaryAction.onClick} variant="outline" asButton>
