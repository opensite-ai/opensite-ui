...
export interface FeatureChecklistThreeColumnProps {
   ...
   */
  checklistColumn1?: string[];
  /**
   * Second column checklist items
   */
  checklistColumn2?: string[];
  /**
...
export function FeatureChecklistThreeColumn({
  ...
  className,
  optixFlowConfig,
}: FeatureChecklistThreeColumnProps) {
  return (
      ...
      <div className="container">
        <div className="grid gap-4 sm:grid-cols-2 sm:gap-8 md:gap-12 lg:grid-cols-3 lg:gap-16">
          ...
          <ul className="flex flex-col gap-3 text-muted-foreground sm:gap-4">
            {checklistColumn1.map((item, index) => (
          ...
          <ul className="flex flex-col gap-4 text-muted-foreground">
            {checklistColumn2.map((item, index) => (
              <li key={index} className="flex items-center gap-2">
                <DynamicIcon name="lucide/check" size={16} className="text-primary" />
                {item}
              </li>
                ...
                <ul className="text-muted-foreground">
                  {card.checklistItems.map((item, itemIndex) => (
                    <React.Fragment key={itemIndex}>
                      ...
                      {itemIndex < card.checklistItems.length - 1 && (
                        <div className="h-px border-t border-dashed"></div>
                      )}
                    </React.Fragment>
                  ))}
