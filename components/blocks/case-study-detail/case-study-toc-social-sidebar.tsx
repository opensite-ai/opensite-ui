...
export function CaseStudyTocSocialSidebar({
  ...
  const heroMediaContent = useMemo(() => {
    ...
    return (
      <Img
        ...
        alt={heroImageAlt}
        className={cn(
          "max-h-[524px] w-full rounded-xl object-cover lg:w-auto",
          heroImageClassName,
        )}
        loading="lazy"
        optixFlowConfig={optixFlowConfig}
  ...
  const sidebarContent = useMemo(() => {
    ...
    return (
      <aside
        className={cn(
          "mx-auto h-fit max-w-prose lg:sticky lg:top-10 lg:mx-0 lg:w-64 lg:max-w-none",
          sidebarClassName,
        )}
      >
        {logoContent}
  ...
  const summaryContent = useMemo(() => {
    ...
    if (!problem && !approach && (!outcomes || outcomes.length === 0))
      return null;

    return (
      <div
        className={cn(
          "grid gap-x-10 gap-y-7 rounded-3xl border p-6 lg:grid-cols-2 lg:gap-y-10 lg:border-none lg:p-0",
          summaryClassName,
        )}
      >
        {problem && (
      ...
      <div className="mt-20">
        {sections.map((section) => (
          <section
            ...
            className={cn(
              getProseClassName(background, "mb-8"),
              section.className,
              proseClassName,
            )}
          >
  ...
  const tocContent = useMemo(() => {
    ...
    return (
      <div
        className={cn(
          "sticky top-8 hidden h-fit shrink-0 lg:block",
          tocClassName,
        )}
      >
            ...
            {sections.map((section) => (
              <li key={section.id}>
                <Pressable
                  href={`#${section.id}`}
                  className={cn(
                    "block py-1 transition-colors duration-200 underline",
  ...
  return (
    ...
    >
      <div className="relative">
        <div
          className={cn(
            "mx-auto flex max-w-prose flex-col items-center justify-between gap-10 lg:max-w-none lg:flex-row",
            headerClassName,
          )}
        >
          <div>
            ...
            {title &&
              (typeof title === "string" ? (
                <h1
                  className={cn(
                    "mt-10 text-5xl font-semibold text-balance lg:text-7xl",
                    titleClassName,
                  )}
                >
                  {title}
        ...
        <div
          className={cn(
            "relative mt-20 flex flex-col gap-x-6 gap-y-16 lg:flex-row",
            layoutClassName,
          )}
        >
          {sidebarContent}
