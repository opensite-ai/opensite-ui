...
interface ContactRsvpFormValues {
  first_name: string;
  last_name: string;
  email: string;
  phone: string;
...
export interface ContactRsvpProps {
  ...
  onSubmit?: (values: ContactRsvpFormValues) => void | Promise<void>;
  /** Success callback */
...
/**
 * ContactRsvp - Contact form with image background layout.
 *
 * @example
 * ```tsx
 * <ContactRsvp
...
export function ContactRsvp({
  ...
  const form = useForm<ContactRsvpFormValues>({
    initialValues: {
      first_name: "",
      last_name: "",
      email: "",
