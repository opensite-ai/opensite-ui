...
interface ContactEventFormValues {
  first_name: string;
  last_name: string;
  email: string;
  phone: string;
...
export interface ContactEventProps {
   ...
   */
  onSubmit?: (values: ContactEventFormValues) => void | Promise<void>;
  /**
   * Success callback
   */
...
/**
 * ContactEvent - Contact form with image background layout.
 *
 * @example
 * ```tsx
 * <ContactEvent
...
export function ContactEvent({
  ...
  onSuccess,
  onError,
}: ContactEventProps): React.JSX.Element {
  const form = useForm<ContactEventFormValues>({
    initialValues: {
      first_name: "",
      last_name: "",
      email: "",
