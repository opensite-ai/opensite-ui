...
interface ContactReferralFormValues {
  first_name: string;
  last_name: string;
  email: string;
  phone: string;
...
export interface ContactReferralProps {
  /** Main heading text */
  ...
  onSubmit?: (values: ContactReferralFormValues) => void | Promise<void>;
  /** Success callback */
...
/**
 * ContactReferral - Contact form with image background layout.
 *
 * @example
 * ```tsx
 * <ContactReferral
...
export function ContactReferral({
  ...
  onSuccess,
  onError,
}: ContactReferralProps): React.JSX.Element {
  const form = useForm<ContactReferralFormValues>({
    initialValues: {
      first_name: "",
      last_name: "",
      email: "",
