...
interface ContactWarrantyFormValues {
  first_name: string;
  last_name: string;
  email: string;
  phone: string;
...
export interface ContactWarrantyProps {
  /** Main heading text */
  ...
  onSubmit?: (values: ContactWarrantyFormValues) => void | Promise<void>;
  /** Success callback */
...
/**
 * ContactWarranty - Contact form with image background layout.
 *
 * @example
 * ```tsx
 * <ContactWarranty
...
export function ContactWarranty({
  ...
  onSuccess,
  onError,
}: ContactWarrantyProps): React.JSX.Element {
  const form = useForm<ContactWarrantyFormValues>({
    initialValues: {
      first_name: "",
      last_name: "",
      email: "",
