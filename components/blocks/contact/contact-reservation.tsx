...
interface ContactReservationFormValues {
  first_name: string;
  last_name: string;
  email: string;
  phone: string;
...
export interface ContactReservationProps {
  ...
  onSubmit?: (values: ContactReservationFormValues) => void | Promise<void>;
  /** Success callback */
...
/**
 * ContactReservation - Contact form with image background layout.
 *
 * @example
 * ```tsx
 * <ContactReservation
...
export function ContactReservation({
  ...
  onSuccess,
  onError,
}: ContactReservationProps): React.JSX.Element {
  const form = useForm<ContactReservationFormValues>({
    initialValues: {
      first_name: "",
      last_name: "",
      email: "",
