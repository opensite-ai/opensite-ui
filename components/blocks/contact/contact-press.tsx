...
interface ContactPressFormValues {
  first_name: string;
  last_name: string;
  email: string;
  phone: string;
...
export interface ContactPressProps {
  /** Main heading text */
  ...
  onSubmit?: (values: ContactPressFormValues) => void | Promise<void>;
  /** Success callback */
...
/**
 * ContactPress - Contact form with image background layout.
 *
 * @example
 * ```tsx
 * <ContactPress
...
export function ContactPress({
  ...
  onSuccess,
  onError,
}: ContactPressProps): React.JSX.Element {
  const form = useForm<ContactPressFormValues>({
    initialValues: {
      first_name: "",
      last_name: "",
      email: "",
