...
describe("HeroStartupLaunchCta", () => {
  ...
  it("preserves empty, false, and zero badge icon behavior", () => {
    const { container, rerender } = render(
      <HeroStartupLaunchCta badge="Featured" badgeIcon="" />,
    );

    expect(screen.queryByTestId("mock-icon-")).not.toBeInTheDocument();

    rerender(<HeroStartupLaunchCta badge="Featured" badgeIcon={false} />);

    expect(screen.queryByTestId("mock-icon-")).not.toBeInTheDocument();

    rerender(<HeroStartupLaunchCta badge="Featured" badgeIcon={0} />);
