...
export interface ProjectVideoHoverGridItem {
  thumbnailSrc: string;
  videoSrc: string;
  studioName: string;
  projectTitle: string;
...
export interface ProjectVideoHoverGridProps {
  /**
   ...
   */
  videoSections?: ProjectVideoHoverGridItem[];
  /**
 ...
 *
 * Displays a header section followed by video cards arranged in a 2-column grid on
 * desktop (single column on mobile). Each card shows a thumbnail that transitions
 * to playing video on hover. Features studio name and project title overlaid on
 * each card. The header includes a bold title and audio icon. Ideal for showcasing
 * multiple video projects in a balanced grid layout while maintaining the immersive
...
export function ProjectVideoHoverGrid({
  ...
  return (
    ...
    >
      <div className={cn("relative h-[50vh]", containerClassName)}>
        <div className={cn("absolute inset-0 bottom-12 flex items-end", headerClassName)}>
