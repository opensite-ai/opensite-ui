...
describe("BannerSurveyIncentive", () => {
  ...
  it("routes main and dismiss icons through DynamicIcon with truthy fallbacks", () => {
    ...
    expect(container.querySelector('[data-name="lucide/survey"]')).toHaveAttribute(
      "data-size",
      "20",
    );
    expect(container.querySelector('[data-name="lucide/survey"]')).toHaveClass(
      "shrink-0",
      "hidden",
      "md:block",
      "custom-icon-class",
    ...
    const mainIconContainer = screen.getByText("Icon banner").parentElement
      ?.parentElement as HTMLElement;
    const dismissButton = screen
      .getByText("Dismiss banner")
      .closest("button") as HTMLElement;
    expect(mainIconContainer).not.toHaveTextContent("lucide/survey");
