...
describe("BannerPrivacyNotice", () => {
  ...
  it("renders with custom props", () => {
    ...
    expect(screen.getByText("View Terms").closest("button")).toHaveAttribute(
      "data-href",
      "#",
    );
  });
  ...
  it("routes main, action, and dismiss icon names without exposing raw text", () => {
    ...
    expect(
      within(action).getByTestId("mock-icon-lucide/file-text"),
    ).toBeInTheDocument();
    expect(
  ...
  it("keeps dismiss behavior and callback intact", () => {
    ...
    render(
      <BannerPrivacyNotice title="Dismiss privacy" onDismiss={onDismiss} />,
    );

    fireEvent.click(screen.getByText("Dismiss banner").closest("button")!);

    expect(onDismiss).toHaveBeenCalledOnce();
    expect(screen.queryByText("Dismiss privacy")).not.toBeInTheDocument();
  });
});
