...
export function PricingMinimalCards({
  ...
  const renderFeatures = (plan: PricingMinimalCardsPlan) => {
    ...
    return (
      <ul className={cn("mb-6 flex-1 space-y-2", featuresClassName)}>
        {plan.features.map((feature, featureIndex) => {
          ...
          return (
            <li key={featureIndex} className={cn("flex items-center gap-2", featureItemClassName, feature.className)}>
              {resolvedIcon}
              {feature.text && (
  ...
  const renderAction = (plan: PricingMinimalCardsPlan) => {
      ...
      <div className={cn("grid gap-6 md:grid-cols-3", gridClassName)}>
        {plans.map((plan, index) => (
          ...
          >
            ...
            <div className="mb-6">
              {plan.price && (
                <span className={cn("text-3xl font-bold", priceClassName)}>{plan.price}</span>
              )}
